...
class ResNetV2(nn.Module):
    ...
    def __init__(self, block_units, width_factor, head_size=2):
        super().__init__()
...
def get_bit_transfer_model(model_name):
    model = KNOWN_MODELS[model_name]()
    model.load_from(get_weights(model_name))
    return model

class FCBiT(nn.Module):
    def __init__(self, wf):
        super(FCBiT, self).__init__()
        self.fc = nn.Sequential(OrderedDict([
            ('gn', nn.GroupNorm(32, 2048*wf)),
            ('relu', nn.ReLU(inplace=True)),
            ('avg', nn.AdaptiveAvgPool2d(output_size=1)),
            ('conv', nn.Conv2d(2048*wf, 2, kernel_size=1, bias=True)),
        ]))
    ...
    def load_from(self, weights, prefix='resnet/'):
        with torch.no_grad():
            self.fc.gn.weight.copy_(tf2th(weights[f'{prefix}group_norm/gamma']))
            self.fc.gn.bias.copy_(tf2th(weights[f'{prefix}group_norm/beta']))
            nn.init.zeros_(self.head.conv.weight)
            nn.init.zeros_(self.head.conv.bias)

KNOWN_WF = OrderedDict([
    ('BiT-M-R50x1', 1),
    ('BiT-M-R50x3', 3),
    ('BiT-M-R101x1', 1),
    ('BiT-M-R101x3', 3),
...
def get_bit_fc_layer(model_name):
    model = FCBiT(KNOWN_WF(model_name))
    model.load_from(get_weights(model_name))
    return model